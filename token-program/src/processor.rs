// The top section defines it as a Solana program, and tells the compiler we want to use the Borsh libraries, as well as the instruction and state modules that we created earlier.
...
//  it defines a process_instruction function, which takes in the standard Solana Rust parameters,
impl Processor {
   pub fn process_instruction(
    //  it retrieves the passed in instruction via the instruction_data parameter, then depending on what value it has (0,1,2,3), it calls different logic to perform the specified function. We’ll fill in details for each instruction next
       _program_id: &Pubkey,
       ...
       match instruction {
           ...
           TokenInstruction::Transfer { amount } => {
              ...
              // it looks for three accounts in the accounts array:
